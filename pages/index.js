...
export default function Home() {
  ...
  const callBackendAPI = async () => {
    const response = await fetch("/api/replicateCall");
    const data = await response.json();
    return data;
  };

  const translate = async () => {
    // const text = `prompt: Generate a pet named ${name} who is similar in feature to a build a bear but is also ${favoriteThings}`;
              ...
              <div className="isolate -space-y-px rounded-md shadow-sm">
                ...
                <div className="relative rounded-md rounded-t-none px-3 pt-2.5 pb-1.5 ring-1 ring-inset ring-gray-300 focus-within:z-10 focus-within:ring-2 focus-within:ring-indigo-600">
                  <label
                    ...
                    className="block text-xs font-medium text-gray-900"
                  >
                    Type three traits you would like your pet to have.
                  </label>
                  <input
                    type="text"
                    name="job-title"
